...
module keyboard_decoder (
  ...
  logic         key_make_strobe_nxt;
  logic [  8:0] key_code_nxt;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      state <= IDLE;
      keys_pressed <= '0;
      key_make_strobe <= 1'b0;
      key_code <= '0;
    end else begin
      state <= state_nxt;
      keys_pressed <= keys_pressed_nxt;
      key_make_strobe <= key_make_strobe_nxt;
  ...
  always_comb begin
    ...
    key_make_strobe_nxt = 1'b0;  // Default zero - forms a 1-cycle pulse
      ...
      case (state)
        IDLE: begin
          if (ps2_rx_data == 8'hF0) begin
            ...
            // Normal make code
            if (!keys_pressed[ps2_rx_data]) begin  // Prevent auto-repeat triggers
              keys_pressed_nxt[ps2_rx_data] = 1'b1;
              key_make_strobe_nxt = 1'b1;
              key_code_nxt = {1'b0, ps2_rx_data};
            end
          end
        end
